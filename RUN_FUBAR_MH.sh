...
for file in $DATADIR/*.msa; do
  ...
  if [[ ! -e $file".FUBAR_MH.json" ]]
  then
      echo Running FUBAR on $file
      #echo hyphy $BATCHFILE --code Universal --alignment $file --tree $file"_NJ.nwk"
      
      $HYPHY $BATCHFILE --code Universal --method Metropolis-Hastings --alignment $file --tree $file"_NJ.nwk" --output $file".FUBAR_MH.json" --cache $file".FUBAR_MH.cache"
  fi
done
